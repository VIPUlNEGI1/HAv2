...
const DoctorDocumentsScreen = () => {
  ...
  // Mock documents - in real app, get from API/store
  const allDocuments: Document[] = [
    ...
    { 
      ...
      size: 2500000,
      type: 'image',
      uri: 'file:///path/to/xray1.jpg',
    },
    { 
      id: '3', 
      patient: 'Jane Smith', 
      patientImage: 'https://i.pravatar.cc/150?u=jane',
      name: 'Prescription_Old.pdf', 
      date: 'Jan 28, 2026', 
      size: 500000,
      type: 'pdf',
      uri: 'file:///path/to/prescription1.pdf',
    },
    { 
      id: '4', 
      patient: 'Jane Smith', 
    ...
    { 
      ...
      size: 5000000,
      type: 'image',
      uri: 'file:///path/to/mri1.jpg',
    },
  ];

  const patients = Array.from(new Set(allDocuments.map(doc => doc.patient))).map(patient => {
    const doc = allDocuments.find(d => d.patient === patient);
    return {
      name: patient,
      image: doc?.patientImage || '',
      count: allDocuments.filter(d => d.patient === patient).length,
    };
  });

  const filteredDocuments = selectedPatient
    ? allDocuments.filter(doc => doc.patient === selectedPatient)
    : allDocuments;
