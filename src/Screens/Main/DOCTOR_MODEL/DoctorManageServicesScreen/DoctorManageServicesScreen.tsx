...
import { useDoctorManageServices, type Service } from './hooks/useDoctorManageServices';

const DoctorManageServicesScreen = () => {
  const { theme, shadows } = useTheme();
  const { services, setServices, availableSlots, setAvailableSlots } = useDoctorManageServices();
  ...
  const updateService = (id: string, field: keyof Service, value: Service[keyof Service]) => {
    setServices(services.map(s => s.id === id ? { ...s, [field]: value } : s));
  };

  const updateSlot = (id: string, field: string, value: string | number | boolean) => {
